...
// Hearthbound Hollow — Editor / Phase46_LaneBoundariesAndWideEnv
//
// Phase 46.2 — Lane boundaries + wider playable environment.
//
...
//
// Phase 46.2 fixes all of that, additively on top of Phase 27.2 and
// Phase 32.2 (their `_Phase27Env_Lane` and `_Phase32Env_Lane` parents are
// preserved untouched), under a new `_Phase46Env_Lane` parent.
//
...
//        extends beyond.
//
// USE: Menu → Hearthbound → ⚙️ Advanced → 🏰 Phase 46.2 — Lane Boundaries + Wide Env
//
// Architecture notes:
...
//     `Assets/_Project/Scripts/Player/LaneGuidePulse.cs` (Phase 46 runtime).
//   - Idempotent: re-running wipes only `_Phase46Env_Lane` and rebuilds.
...
{
    public static class Phase46_LaneBoundariesAndWideEnv
    {
        ...
        private const string EnvParentName = "_Phase46Env_Lane";

        [MenuItem("Hearthbound/⚙️ Advanced/🏰 Phase 46.2 — Lane Boundaries + Wide Env", priority = 462)]
        public static void Build()
        {
            if (!System.IO.File.Exists(SceneLane))
            {
                EditorUtility.DisplayDialog("Phase 46.2",
            ...
            // Wipe previous Phase 46.2 pass — idempotent.
            ...
            EditorUtility.DisplayDialog("Phase 46.2 — Lane Boundaries Built",
        ...
        private static int BuildPerimeterWalls(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            ...
            if (wallPrefab == null)
            {
                Debug.LogWarning("[Hearthbound/Phase 46.2] No wall prefab found in Medieval Village. Skipping visible perimeter; relying on invisible blockers.");
                return 0;
            }
