...
// Hearthbound Hollow — Editor / Phase46_CottageInteriorPolish
//
// Phase 46.5 — Mission 2 Widower's Cottage interior polish.
//
...
//
// Phase 46.5 adds the props the earlier capstones didn't cover, all
// procedurally + idempotently under `_Phase46Env_Cottage`:
//
...
//   4. The framed photograph on the mantel — Margery + Gerrold wedding
//      day (procedural sprite using a Bamao parchment frame proxy).
...
//   9. Two mantel candles + a Lumen halo (the cottage is warm — Doris's
//      kindness made it so).
//   10. The closed bedroom door (decorative — locked in M1-2).
//
// USE: Menu → Hearthbound → ⚙️ Advanced → 🏚️ Phase 46.5 — Cottage Interior Polish
...
{
    public static class Phase46_CottageInteriorPolish
    {
        ...
        private const string EnvParentName = "_Phase46Env_Cottage";

        [MenuItem("Hearthbound/⚙️ Advanced/🏚️ Phase 46.5 — Cottage Interior Polish", priority = 465)]
        public static void Build()
        {
            if (!System.IO.File.Exists(SceneCottage))
            {
                EditorUtility.DisplayDialog("Phase 46.5",
            ...
            EditorUtility.DisplayDialog("Phase 46.5 — Cottage polished",
        ...
        // ───────────────────────────────────────────────────────────────
        // 1) Harden existing wall colliders
        // ───────────────────────────────────────────────────────────────
        ...
        // ───────────────────────────────────────────────────────────────
        // 3) Hearth fire — Lumen flame + flickering point light
        // ───────────────────────────────────────────────────────────────
        ...
        private static int BuildHearthFire(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            ...
            // Realtime flickering point light
            ...
            light.color = new Color(1.0f, 0.59f, 0.31f); // hearth orange
            ...
            // Lumen candle / fire effect (best-effort)
        ...
        // ───────────────────────────────────────────────────────────────
        // 4) Framed photograph on the mantel
        // ───────────────────────────────────────────────────────────────

        private static int BuildFramedPhotograph(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            // Procedural picture frame — a thin cube with a sepia-toned material.
            // (Until a proper photo asset is imported.)
            ...
            // Remove collider (it's on a mantel, not an obstacle)
            ...
            if (mr != null)
            {
                ...
                mat.name = "Phase46_PhotoFrame";
                mat.color = new Color(0.82f, 0.66f, 0.45f, 1f); // sepia photo + wood frame
                mr.sharedMaterial = mat;
            }

            // 3D label so the cozy player understands what it is.
        ...
        // ───────────────────────────────────────────────────────────────
        // 6) Margery's chair (with book on arm)
        // ───────────────────────────────────────────────────────────────
        ...
        // ───────────────────────────────────────────────────────────────
        // 8) Wool rug between the two chairs
        // ───────────────────────────────────────────────────────────────
        ...
        private static int BuildRug(Transform envRoot)
        {
            ...
            {
                var mat = new Material(Shader.Find("Universal Render Pipeline/Lit") ?? Shader.Find("Standard"));
                mat.name = "Phase46_CottageRug";
                mat.color = new Color(0.42f, 0.32f, 0.25f, 1f); // muted oak/brown
        ...
        // ───────────────────────────────────────────────────────────────
        // 10) Bedroom door (closed; lit underneath; locked in M1-2)
        // ───────────────────────────────────────────────────────────────

        private static int BuildBedroomDoor(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            int placed = 0;
            // V2 bindings don't expose a door slot; use fuzzy search directly.
            // Door prefab keywords order: tightest match first.
            ...
            // Floating "locked in M1-2" hint label
            ...
            // Subtle warm sliver of light under the door (point light)
