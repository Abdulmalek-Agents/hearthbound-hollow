...
// Hearthbound Hollow — Editor / Phase47_GardenBoundariesAndPath
//
// Phase 47.4 — Mission 2 Herb Garden boundaries + wider stepping path.
//
...
// walled off beyond. Phase 47.4 ensures:
//
...
//
// All additions are parented under `_Phase47Env_Garden` — earlier-phase
// outputs are preserved.
//
// USE: Menu → Hearthbound → ⚙️ Advanced → 🌿 Phase 47.4 — Garden Boundaries + Path
...
{
    public static class Phase47_GardenBoundariesAndPath
    {
        ...
        private const string EnvParentName = "_Phase47Env_Garden";

        // Garden playable area: 16m × 18m
        private const float HalfWidth   = 8f;
        private const float ZSouth      = -2f;
        private const float ZNorth      = 16f;
        private const float WallHeight  = 1.8f;

        [MenuItem("Hearthbound/⚙️ Advanced/🌿 Phase 47.4 — Garden Boundaries + Path", priority = 474)]
        public static void Build()
        {
            if (!System.IO.File.Exists(SceneGarden))
            {
                EditorUtility.DisplayDialog("Phase 47.4",
            ...
            EditorUtility.DisplayDialog("Phase 47.4 — Garden polished",
