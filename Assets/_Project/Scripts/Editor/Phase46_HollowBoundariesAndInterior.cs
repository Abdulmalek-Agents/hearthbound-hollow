...
// Hearthbound Hollow — Editor / Phase46_HollowBoundariesAndInterior
//
// Phase 46.3 — Hollow interior wall colliders + cozy interior polish.
//
...
// and got rejected by URP at runtime). Phase 46.3:
...
//
// All additions are parented under `_Phase46Env_Hollow` — Phase 27.3 and
// Phase 32.3 outputs are preserved.
//
// USE: Menu → Hearthbound → ⚙️ Advanced → 🏠 Phase 46.3 — Hollow Boundaries + Interior Polish
...
{
    public static class Phase46_HollowBoundariesAndInterior
    {
        ...
        private const string EnvParentName  = "_Phase46Env_Hollow";

        [MenuItem("Hearthbound/⚙️ Advanced/🏠 Phase 46.3 — Hollow Boundaries + Interior Polish", priority = 463)]
        public static void Build()
        {
            if (!System.IO.File.Exists(SceneHollow))
            {
                EditorUtility.DisplayDialog("Phase 46.3",
            ...
            EditorUtility.DisplayDialog("Phase 46.3 — Hollow polished",
        ...
        // ───────────────────────────────────────────────────────────────
        // 1) Harden wall colliders on every existing wall GameObject.
        // ───────────────────────────────────────────────────────────────
        ...
        private static int HardenWallColliders()
        {
            ...
            foreach (var go in allObjs)
            {
                ...
                // Skip the door — has its own collider + interactable.
                if (lower.Contains("door")) continue;

                // Already has a collider?
                if (go.GetComponent<Collider>() != null) continue;

                // Has a renderer? Add a BoxCollider sized to its mesh bounds.
        ...
        // ───────────────────────────────────────────────────────────────
        // 2) Invisible room boundary (defence-in-depth — player can't escape)
        // ───────────────────────────────────────────────────────────────
        ...
        private static int BuildRoomBoundary(Transform envRoot)
        {
            ...
            // The Hollow's two rooms together are roughly 18 m × 11 m.
            // Drop 4 thin invisible walls 1 m outside the visible perimeter.
            float halfW = 9.5f;
            float halfL = 6.0f;
            float zCentre = 0f; // approximate centre of the shop+workbench rooms
        ...
        private static int PlaceRug(Transform envRoot)
        {
            ...
            // Remove the default collider (rug should not block movement)
            ...
            if (mr != null)
            {
                ...
                mat.name = "Phase46_RugWool";
                mat.color = new Color(0.55f, 0.20f, 0.18f, 1f); // deep burgundy
        ...
        // ───────────────────────────────────────────────────────────────
        // 4) Bookcase against the east wall
        // ───────────────────────────────────────────────────────────────
        ...
        private static int PlaceBookcase(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            ...
            // Drop 3-5 stacked books on top + on shelves.
        ...
        // ───────────────────────────────────────────────────────────────
        // 5) Second reading chair beside the existing one
        // ───────────────────────────────────────────────────────────────
        ...
        private static int PlaceReadingChair(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            ...
            // Drape a small folded blanket on the chair (proxy: a sack prefab)
        ...
        // ───────────────────────────────────────────────────────────────
        // 6) Writing desk with quill + ledger
        // ───────────────────────────────────────────────────────────────
        ...
        // ───────────────────────────────────────────────────────────────
        // 7) Wall candle sconces above the door
        // ───────────────────────────────────────────────────────────────
        ...
        private static int PlaceWallSconces(Transform envRoot, MedievalVillageBindingsV2 binds)
        {
            ...
            // Mount one candle on each side of the door.
            for (int i = 0; i < 2; i++)
            {
                ...
                // Real point light for the candle.
        ...
        // ───────────────────────────────────────────────────────────────
        // 8) Hanging dried-lavender bundle (foreshadows Mission 2)
        // ───────────────────────────────────────────────────────────────

        private static int PlaceHangingLavender(Transform envRoot)
        {
            // Simple procedural: a small cube tinted purple, suspended at ~2.6 m
            // along the hallway between shop and workbench.
            ...
            {
                var mat = new Material(Shader.Find("Universal Render Pipeline/Lit") ?? Shader.Find("Standard"));
                mat.name = "Phase46_DriedLavender";
