...
namespace FIMSpace.FEyes
{
    public partial class FEyesAnimator
    {
        ...
        /// <summary> Resetting blinking timers for all eyes </summary>
        public void ForceBlink()
        {
            ...
            for( int i = 0; i < blinking.Length; i++ )
            {
                if( blinking[i] == null ) continue;
                blinking[i].timer = 0f;
                blinking[i].keepCloseTime = 0f;
                blinking[i].progress = 0f;
            }
        }

        /// <summary> Reset blinking timer for the single eye </summary>
        public void ForceBlinkEye( int eyeIndex )
        {
            if( blinking == null ) return;
            if( eyeIndex < 0 || eyeIndex >= blinking.Length ) return;
            if( blinking[eyeIndex] == null ) return;
            blinking[eyeIndex].timer = 0f;
            blinking[eyeIndex].keepCloseTime = 0f;
            blinking[eyeIndex].progress = 0f;
        }
