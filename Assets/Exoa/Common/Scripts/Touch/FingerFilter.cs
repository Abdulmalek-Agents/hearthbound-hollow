...
namespace Exoa.Touch
{
    ...
    public class FingerFilter
    {
        ...
        public TouchSelectable RequiredSelectable;

        [System.NonSerialized]
        private List<TouchFinger> fingers;

        [System.NonSerialized]
        private List<TouchFinger> filteredFingers;

        public FingerFilter(bool newIgnoreStartedOverGui) : this(default(FilterType), newIgnoreStartedOverGui, default(int), default(int), default(TouchSelectable))
        {
        }

        public FingerFilter(FilterType newFilter, bool newIgnoreStartedOverGui, int newRequiredFingerCount, int newRequiredMouseButtons, TouchSelectable newRequiredSelectable)
        {
            Filter = newFilter;
        ...
        public void UpdateRequiredSelectable(GameObject gameObject)
        {
            ...
            {
                RequiredSelectable = gameObject.GetComponentInParent<TouchSelectable>();
            }
        }

        /// <summary>If you've set Filter to ManuallyAddedFingers, then you can call this method to manually add a finger.</summary>
        public void AddFinger(TouchFinger finger)
        {
            if (finger != null)
            {
                if (fingers == null)
                {
                    fingers = new List<TouchFinger>();
                }
                else
                {
        ...
        public void RemoveFinger(TouchFinger finger)
        {
            if (fingers != null)
            {
                for (var i = fingers.Count - 1; i >= 0; i--)
        ...
        public List<TouchFinger> UpdateAndGetFingers(bool ignoreUpFingers = false)
        {
            if (filteredFingers == null)
            {
                filteredFingers = new List<TouchFinger>();
            }
            ...
            {
                filteredFingers.AddRange(SelectableByFinger.GetFingers(IgnoreStartedOverGui, false, 0, RequiredSelectable));
            }
            ...
            if (RequiredMouseButtons > 0 && SimulatedFingersExist(filteredFingers) == true)
            {
                for (var i = 0; i < 5; i++)
                {
                    ...
                    if ((RequiredMouseButtons & mask) != 0 && BaseTouchInput.GetMouseIsHeld(i) == false && BaseTouchInput.GetMouseWentUp(i) == false)
                    {
                        filteredFingers.Clear();
                    }
                }
        ...
        private static bool SimulatedFingersExist(List<TouchFinger> fingers)
        {
...
#if UNITY_EDITOR
namespace Exoa.Touch.Editor
{
    using UnityEditor;

    [CustomPropertyDrawer(typeof(FingerFilter))]
    public class LeanFingerFilter_Drawer : PropertyDrawer
    {
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            var filter = (FingerFilter.FilterType)property.FindPropertyRelative("Filter").enumValueIndex;
            var height = base.GetPropertyHeight(property, label);
            var step = height + 2;

            switch (filter)
            {
                case FingerFilter.FilterType.AllFingers: height += step * 4; break;
            }
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            EditorGUI.indentLevel++;
            {
                switch (filter)
                {
                    case FingerFilter.FilterType.AllFingers:
                        {
