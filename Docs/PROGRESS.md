...
- **FREEZE (blocker) — fixed at the root.** Read the TMP source: the exception is at
  `TextMeshProUGUI.cs:2886`, `(TMP_SpriteCharacter)textInfo.characterInfo[…].textElement` — a
  Unity 6 TMP type-confusion that fires whenever an emoji **`<sprite>`** is mixed with the Arabic
  **font fallback** in one mesh. The trigger wasn't just the Help body: the **always-on control-hint
  chips** (WASD/E/H captions) ran `HollowGlyphs.Format("✋"/"🚶"/"❓")` → sprite + Arabic caption, so
  the cast threw **every canvas update**, freezing the game. Fix: `HollowGlyphs.Format` now **strips
  decorative emoji to clean text and never emits `<sprite>`** (+ drops stray surrogate-pair emoji),
  so no text resolves an emoji to a sprite → the cast at 2886 never runs. The Help controls list is
  also emoji-free. [HollowGlyphs.cs](Assets/_Project/Scripts/UI/HollowGlyphs.cs),
  [HelpOverlayUI.cs](Assets/_Project/Scripts/UI/HelpOverlayUI.cs).
  *(Trade-off: the gold emoji glyphs are gone for now — stability over decoration. Re-enable once
  the sprite-asset / TMP version issue is properly resolved. If a crash ever recurs from raw emoji
  in a HUD I didn't route through Format, the escalation is unregistering the HollowGlyphs sprite
  asset from TMP Settings.)*
