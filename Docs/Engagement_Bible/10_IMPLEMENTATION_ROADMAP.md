...
| **62** | P2 | `RequestSO`/`RequestPoolSO`, `RequestBoardService`, `RequestBoardUI`, generalize directors → `VisitDirector`, re-express Doris+Gerrold as data, `AlmanacSO` | ✅ Done |
| **63** | P6 | `EchoSO`/`EchoWebService`, promote `MemoryWebOverlay` → Memory Wall, wire thread rewards | ✅ Done |
| **64** | P3 | `HollowUpgradeSO` + catalog, `HollowProgressionService`, `HollowShopUI`, coin-purse HUD, pre-placed hidden upgrade markers, wire transactions→coin | ✅ Done |
| **65** | P4 | `GardenBedState` + `GardenService`, swap to HarvestGarden staged-growth, teas-as-tools, seed/tea economy | ✅ Done |
| **66** | P5 | Juice pass on Polish/Cleanse, `CraftVerb` + `acceptedVerbs`, Sort + Steep verbs, Keeper's Hand mastery flavor | ✅ Done |
| **67** | P7 | Almanac festivals (Honey / Hearth), Market Day cart, birthdays, bard visits; redress scenes | ✅ Done |
| **68** | P6+ | Author 4–6 procedural villagers via the Vignette Library; expand the Request pool | ✅ Done (built-in roster/pools; authored SOs extend w/ zero code) |
| **69** | all | Cozy feedback pass (D-076): coin purse, collection %, agenda — all celebratory, comfort-gated | 🟡 Core done |
| **70** | all | **G-Engage playtest build** — 3 looped days; instrument the "voluntary Day 4" metric | 🟡 Plan shipped (`Docs/PHASE70_GENGAGE_PLAYTEST.md`) |
| **71** | all | Tune from playtest; pace heavy beats (Comedy/Grief radius across the loop) | 🟡 Guidance documented |
| **72** | all | Marketing-truth pass: update README/Steam copy to match the now-real loop (kill the refund-driving mismatch) | 🟡 `Docs/MARKETING_TRUTH_Phase72.md` |
...
P1 is the root. **Pillars P1–P7 are now all live** (Phases 61–67); the remaining work is
validation + tuning (G-Engage, Phase 70–72). See `11_WIDER_CONTENT_EXECUTION_PLAN.md` for the
mandate-level tracker.
...
### Phase 62–67 — The cozy daily loop, end to end 🟢 (2026-05-30)
The owner directed the studio to extend beyond M1-2 and **un-defer the loop**. Delivered the six
remaining pillars as self-installing, fallback-safe, asmdef-clean systems (full detail in
`11_WIDER_CONTENT_EXECUTION_PLAN.md` + `Docs/PROGRESS_Phase62_67_EngagementLoop.md`):
- **62 (P2):** interactive Request Board ([B]) + `RequestVisitService` + `VillageStateFlags` (D-079)
  + the real coin economy; save schema v3 so the loop compounds.
- **63 (P6):** Memory Wall ([M]) + `EchoWebService` + `EchoBoard` — Echo threads to chase/complete.
- **64 (P3):** `HollowProgressionService` + shop ([U]) + coin-purse HUD — coin → cozy upgrades.
- **65 (P4):** `GardenService` + Garden ([G]) — grow→brew→use teas (tools) + garden→tea→coin wheel.
- **66 (P5):** `WorkbenchService` + bench ([K]) + `CraftVerbs` — varied verbs + Keeper's-Hand mastery.
- **67 (P7):** `AlmanacService` — Market Day / Festival / bard / birthday anticipation in the Agenda.
- **68:** procedural content **un-deferred** via built-in rosters/echoes/catalog/herbs (authored SO
  pools extend with zero code). **69:** cozy feedback core (coin purse, Agenda growth footer, `[J]`).
- New decisions **D-079 / D-080 / D-081**. UI↔Mission bridged via Core blackboards (`DayAgenda`,
  `EchoBoard`, `HollowShopBoard`, `GardenBoard`) + intent events — **no UI→Mission edge** (D-035 held).
...
- **D-079 (Phase 62)** — String-flag content gating via reflection (`VillageStateFlags`): designers gate requests/echoes by VillageState bool *name* with zero code.
- **D-080 (Phase 63)** — The Memory Wall ships as a companion self-installing screen ([M]) rather than mutating the Phase-51 scene-built Tab overlay; both can be unified later.
- **D-081 (Phases 62–67)** — Every Engagement-loop screen is self-installing + fallback-safe, and bridges UI↔Mission via Core blackboards (`DayAgenda`/`EchoBoard`/`HollowShopBoard`/`GardenBoard`) + intent events — preserving the asmdef graph (no UI→Mission edge) and the pull→Build-Everything→Play workflow (no scene edits required).
...
cozy home players live in. **(Pillars P1–P7 are live as of Phase 67; G-Engage validation is next.)**

---

*Roadmap v1.1 — `feat/mission-1-2-architecture` · 2026. End of the Engagement Bible.*
